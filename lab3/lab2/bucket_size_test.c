...
double *measure_for_threads(int threads, int bucket_multiplier)
{
    ...
    int bucket_size = 10 * bucket_multiplier;
...
#pragma omp barrier
    ...
    // times[BUCKET_DISTRIBUTION] = omp_get_wtime() - phase_start;
    // bucket sort
    phase_start = omp_get_wtime();
...
/**
 * Function, that joins below arrays and saves below times to one csv file
 * double total_time[max_threads];
    double numbers_generation_time[max_threads];
    double bucket_distibution_time[max_threads];
    double bucket_sort_time[max_threads];
    double bucket_join_time[max_threads];*/
...
int main(int argc, char *argv[])
{
    ...
    /* Times */
    double *total_time = initialize_empty_array(multiplier);
    double *numbers_generation_time = initialize_empty_array(multiplier);
    double *bucket_distibution_time = initialize_empty_array(multiplier);
    double *bucket_sort_time = initialize_empty_array(multiplier);
    double *bucket_join_time = initialize_empty_array(multiplier);
    /**/
    ...
    for (i = 1; i <= 1; ++i)
    {
        ...
        {
            double *experiment_times = measure_for_threads(i, multiplier);
            total_time[j - 1] += experiment_times[TOTAL_TIME];
            numbers_generation_time[j - 1] += experiment_times[GENERATION];
            bucket_distibution_time[j - 1] += experiment_times[BUCKET_DISTRIBUTION];
            bucket_sort_time[j - 1] += experiment_times[BUCKET_SORT];
            bucket_join_time[j - 1] += experiment_times[BUCKET_JOIN];
            free(experiment_times);
            save_to_file("bucket_size_tests.csv", multiplier, total_time, numbers_generation_time, bucket_distibution_time, bucket_sort_time, bucket_join_time);
        }
    }
    free(total_time);
