...
double *measure_for_threads(int threads, int bucket_multiplier)
{
    ...
    int table_size = 5e8; // 7;
    int bucket_val_range = 10;
...
#pragma omp parallel for num_threads(threads)                           \
    shared(buckets, to_sort, bucket_val_range, bucket_sizes) private(i) \
        firstprivate(threads, table_size, bucket_count)

    for (i = 0; i < table_size; i++)
    {
        double val = to_sort[i];
        int macthing_bucket = (int)val / bucket_val_range;
...
#pragma omp parallel for num_threads(threads)                           \
    ...
    for (i = 0; i < table_size; i++)
    {
        ...
        int bucket_index = i / bucket_val_range;

        int start_idx = first_el_idx(buckets[bucket_index]);
        int bucket_size = bucket_sizes[bucket_index];

        // printf("Bucket %d, bucket_size: %d, value: %f", i, bucket_sizes[i], buckets[i][start_idx]);
        for (j = start_idx; j < bucket_size; j++)
        {
            // printf(" %f", buckets[i][j]);
            to_sort[i] = buckets[bucket_index][j];
        }
        // printf("\n");
    }
...
int main(int argc, char *argv[])
{
    ...
    save_to_file("fk_results.csv", max_threads, total_time, numbers_generation_time, bucket_distibution_time, bucket_sort_time, bucket_join_time);
    free(total_time);
